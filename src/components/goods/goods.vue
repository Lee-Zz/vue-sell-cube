<template>
  <div class="goods">
    <div ref="wrapper1">
      <div class="good_left" >
        <div v-for="(item,index) in foodData" :key="index"  :class="{'current':currentIndex === index}" class="good_left_box">
          {{item.name}}
        </div>
      </div>
    </div>
    ...
    <div ref="wrapper2">
      <div class="good_right" >
        <div v-for="(item, index) in foodData" :key="index" ref="footlist">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed:{
      currentIndex(){  
        console.log(this.listHeight,'thislistHeight')
    ...
    methods: {
     _initScroll(){
      ...
      this.foodsScroll = new BScroll(this.$refs.wrapper2,{
        probeType:3 // 在滚动的时候能够实时的告诉我们滚动的位置
      });
     ...
     _calculateHeight(){
      let footlist = this.$refs.footlist 
      let height = 0 
